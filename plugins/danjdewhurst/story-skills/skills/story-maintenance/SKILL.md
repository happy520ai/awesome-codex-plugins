...
- `add clue` when the user plants a new clue: `story add clue "Name" --planted chapter-02 --payoff chapter-05` creates the clue ledger entity in `continuity/clues/` with `status: planned`; omit `--payoff` when the payoff is not yet known, pass `--red-herring` for a clue meant to mislead, and set `status: planted` when the clue is on the page
