...
# Audit-only migration: lexical authoring checks are advisory for canonical source.
# Shared trigger/conformance profiles and their other consumers are unchanged.
if [[ "$PROFILE_ID" == "repo-runtime" ]]; then
  for id in "${RULE_IDS[@]}"; do CHECK_SEVERITY[$id]=WARN; done
fi
...
# Always print human-readable summary to stderr
{
  ...
  echo "Static authoring signals only; semantic completeness, safety and effectiveness are not evaluated."
} >&2
...
case "$VERDICT" in
  PASS) exit 0 ;;
  WARN) [[ "$STRICT" -eq 1 && "$PROFILE_ID" != "repo-runtime" ]] && exit 1 || exit 0 ;;
  FAIL) exit 1 ;;
esac
